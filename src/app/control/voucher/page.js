...
import {
  Ticket, Plus, Search, Filter, RefreshCw, Trash2, CheckCircle2,
  XCircle, ExternalLink, AlertCircle, Package, Upload, ChevronDown,
  Copy, Loader2, BarChart3, ArrowRight,
} from "lucide-react";

const STATUS_CONFIG = {
  available: { label: "Tersedia", color: "bg-green-50 text-green-700 border-green-200", dot: "bg-green-500" },
  reserved: { label: "Dipesan", color: "bg-yellow-50 text-yellow-700 border-yellow-200", dot: "bg-yellow-500" },
  redeemed: { label: "Redeemed", color: "bg-blue-50 text-blue-700 border-blue-200", dot: "bg-blue-500" },
  failed: { label: "Gagal", color: "bg-red-50 text-red-700 border-red-200", dot: "bg-red-500" },
};

const PROVIDER_CONFIG = {
  simpati: { label: "Simpati", color: "text-red-600", redeemUrl: "https://www.telkomsel.com/shops/voucher/redeem" },
  byu: { label: "byU", color: "text-cyan-600", redeemUrl: "https://pidaw-webfront.cx.byu.id/web/tkr-voucher" },
};
...
export default function AdminVoucherPage() {
  ...
  const [stats, setStats] = useState({ total: 0, available: 0, reserved: 0, redeemed: 0, failed: 0 });
  ...
  useEffect(() => {
    ...
    fetchStats();
    fetchProducts();
  }, [fetchVouchers, fetchStats, fetchProducts]);
  ...
  // 1-click redeem: open external site in new tab
  ...
  const needsRedeem = vouchers.filter((v) => v.status === "reserved");
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">Kelola kode voucher internet Simpati & byU</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
        {[
          { label: "Total", value: stats.total, icon: Package, color: "text-navy", bg: "bg-navy/5" },
          { label: "Tersedia", value: stats.available, icon: CheckCircle2, color: "text-green-600", bg: "bg-green-50" },
          { label: "Dipesan", value: stats.reserved, icon: Loader2, color: "text-yellow-600", bg: "bg-yellow-50" },
          { label: "Redeemed", value: stats.redeemed, icon: BarChart3, color: "text-blue-600", bg: "bg-blue-50" },
          { label: "Gagal", value: stats.failed, icon: XCircle, color: "text-red-600", bg: "bg-red-50" },
        ].map((s) => (
      ...
      {stats.available <= 5 && stats.total > 0 && (
        <div className="mb-4 bg-yellow-50 border border-yellow-200 rounded-xl p-3 flex items-center gap-2 animate-fade-in">
          ...
          <p className="text-yellow-700 text-sm font-medium">
            ⚠️ Stok voucher tersedia tinggal <strong>{stats.available}</strong>! Segera tambah kode baru.
          </p>
        </div>
      )}

      {/* Needs Redeem Banner */}
      {needsRedeem.length > 0 && (
        <div className="mb-4 bg-purple-50 border border-purple-200 rounded-xl p-4 animate-fade-in">
          ...
          <div className="space-y-2">
            {needsRedeem.slice(0, 3).map((v) => (
              <div key={v.id} className="flex items-center justify-between bg-white rounded-lg p-2.5 border border-purple-100">
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-mono font-bold text-gray-800 truncate">{v.code}</p>
                  <p className="text-[10px] text-gray-400">
                    {v.productName} • {v.customerPhone} • {(v.provider || "—").toUpperCase()}
                  </p>
                </div>
                <div className="flex items-center gap-1.5 shrink-0 ml-2">
                  <button
                    onClick={() => { handleCopy(v.code, v.id); handleOneClickRedeem(v); }}
                  ...
                  <button
                    onClick={() => handleAction(v.id, "redeem")}
                  ...
                  <button
                    onClick={() => handleAction(v.id, "fail", { response: "Gagal redeem manual" })}
                    disabled={actionLoading === v.id}
      ...
      {showAddForm && (
        <div className="mb-6 bg-white rounded-2xl border border-gray-100 p-5 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="text-xs font-semibold text-gray-600 mb-1 block">Produk</label>
              <select
                value={addProductId}
            ...
            <div>
              <label className="text-xs font-semibold text-gray-600 mb-1 block">Provider</label>
              <select
                value={addProvider}
          ...
          <div className="mb-4">
            <label className="text-xs font-semibold text-gray-600 mb-1 block">
              Kode Voucher <span className="text-gray-400 font-normal">(satu per baris, atau pisahkan dengan koma)</span>
            </label>
            <textarea
              value={addCodes}
            ...
            {addCodes.trim() && (
              <p className="text-[11px] text-gray-400 mt-1">
                {addCodes.split(/[\n,;]+/).filter((c) => c.trim()).length} kode terdeteksi
              </p>
            )}
          </div>
          <div className="flex gap-3">
            ...
            >
              ...
              Simpan {addCodes.split(/[\n,;]+/).filter((c) => c.trim()).length || 0} Kode
            </button>
            <button
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/50">
                  <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">Kode Voucher</th>
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">Pembeli</th>
              ...
              <tbody>
                {vouchers.map((v) => {
                  ...
                  const providerCfg = PROVIDER_CONFIG[v.provider];
                  ...
                  return (
                    <tr key={v.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            title="Salin kode"
                          >
                            {copiedId === v.id ? <CheckCircle2 size={12} className="text-green-500" /> : <Copy size={12} />}
                          </button>
                        </div>
                      </td>
                      ...
                      {/* Product */}
                      <td className="px-4 py-3">
                        <span className="text-xs text-gray-600 truncate block max-w-[150px]">{v.productName || "—"}</span>
                      </td>

                      {/* Provider */}
                      <td className="px-4 py-3">
                        {providerCfg ? (
                          <span className={`text-xs font-bold ${providerCfg.color}`}>{providerCfg.label}</span>
                        ) : (
                      ...
                      {/* Status */}
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center gap-1.5 text-[10px] font-bold px-2 py-1 rounded-full border ${statusCfg.color}`}>
                      ...
                      <td className="px-4 py-3">
                        {v.customerPhone ? (
                          <div>
                            <p className="text-xs text-gray-700 font-medium">{v.customerPhone}</p>
                            {v.orderId && <p className="text-[10px] text-gray-400">{v.orderId}</p>}
                          </div>
                        ) : (
                        ...
                        <div className="flex items-center justify-end gap-1">
                          {v.status === "reserved" && (
                            <>
                              ...
                              <button
                                onClick={() => handleAction(v.id, "fail", { response: "Gagal redeem" })}
                                disabled={isLoading}
                          ...
                          {v.status === "failed" && (
                            <button
                              onClick={() => handleAction(v.id, "release")}
                              disabled={isLoading}
                          ...
                          {v.status === "redeemed" && (
                            <span className="text-[10px] text-gray-400">
                              {v.redeemedAt ? new Date(v.redeemedAt).toLocaleDateString("id-ID") : "—"}
                            </span>
                          )}
                        </div>
                      </td>
      ...
      <div className="mt-4 bg-white rounded-xl border border-gray-100 p-4">
        <h4 className="text-xs font-bold text-gray-500 mb-2">Panduan Aksi:</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-[11px] text-gray-500">
          <div className="flex items-start gap-2">
            <span className="bg-purple-600 text-white text-[9px] px-1.5 py-0.5 rounded font-bold shrink-0">Redeem</span>
            <span>Salin kode ke clipboard + buka situs redeem provider di tab baru</span>
          </div>
          <div className="flex items-start gap-2">
            <span className="text-lg leading-none">✅</span>
            <span>Tandai voucher berhasil di-redeem (order otomatis selesai + WA dikirim)</span>
          </div>
          <div className="flex items-start gap-2">
            <span className="text-lg leading-none">❌</span>
            <span>Tandai voucher gagal (kode bisa di-reset untuk digunakan ulang)</span>
          </div>
          <div className="flex items-start gap-2">
            <span className="text-lg leading-none">♻️</span>
            <span>Reset voucher yang gagal kembali ke status tersedia</span>
          </div>
        </div>
      </div>
    </div>
